...
VideoStream.prototype.onSocketConnect = function (socket, request) {
  ...
  return socket.on('close', (code, message) => {
    ...
    if (this.wsServer.clients.size === 0) {
      this.stop();
      delete global.streams[this.wsPort]  // Delete port from global array when there are no clients connected
    }
    else global.streams[this.wsPort]--; // Decrease the number of clients connected to the current port in global array
    console.log("Clients connected: ", global.streams);
  });
};
